...
const cvsFiltration = (arr) => {
    ...
    for (let i=0; i < arr.length; i++){
        if(arr[i].yearsOfExperience > 4 && arr[i].tech === "JS"){
            result.push({
                   fullName:arr[i].firstName + " " + arr[i].LastName, tech:arr[i].tech
                })
        }
    }
    return result 
...
const skillComparison = (arr1, arr2) => {
    ...
    let result = [];
    const countsArray = {};
    const bothOfThem = arr1.concat(arr2); 

    for (const count of bothOfThem) {
        countsArray[count] = countsArray[count] ? countsArray[count] + 1 : 1;
    }
    for (const course in countsArray) {
        if(countsArray[course] == 1){
            result.push(course)
        }
    }
    return result;
